...
export default function MetaCallback() {
  ...
  const exchangeCode = async (code) => {
    try {
      ...
      setStatus('success')
      setMessage(`¡Conectado! ${response.data.accounts_count || ''} cuenta(s) encontrada(s)`)

      // Notificar a la ventana padre si es un popup
      if (window.opener) {
        window.opener.postMessage({ 
          type: 'META_AUTH_SUCCESS',
          accounts: response.data.accounts_count
        }, window.location.origin)
      ...
      setStatus('error')
      setMessage(err.message || 'Error al conectar con Meta Ads')
