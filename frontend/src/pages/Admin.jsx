...
export default function Admin() {
  ...
  const loadData = async () => {
    ...
    try {
      if (activeTab === 'codes') {
        const response = await api.get('/auth/admin/invite-codes')
  ...
  const createCode = async () => {
    ...
    try {
      const response = await api.post('/auth/admin/invite-codes', {
        max_uses: maxUses,
  ...
  const deleteCode = async (codeId) => {
    ...
    try {
      await api.delete(`/auth/admin/invite-codes/${codeId}`)
