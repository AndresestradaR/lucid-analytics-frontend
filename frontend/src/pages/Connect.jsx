...
export default function Connect() {
  ...
  useEffect(() => {
    ...
    const handleMessage = (event) => {
      if (event.data?.type === 'META_AUTH_SUCCESS') {
        setSuccess('¡Meta Ads conectado exitosamente!')
        checkConnections()
      } else if (event.data?.type === 'META_AUTH_ERROR') {
        setError(event.data.message || 'Error al conectar Meta Ads')
      }
    }
  ...
  const connectMeta = () => {
    ...
    // Verificar que existe el META_APP_ID
    if (!clientId) {
      setError('Error de configuración: META_APP_ID no está configurado. Contacta al administrador.')
      return
    }
    ...
    // Si el navegador bloqueó el popup, abrir en nueva pestaña
    if (!popup || popup.closed) {
      window.open(authUrl, '_blank')
    }
  }

  const disconnectMeta = async (accountId) => {
    setLoading(prev => ({ ...prev, meta: true }))
    try {
      await api.delete(`/meta/disconnect/${accountId}`)
      setSuccess('Cuenta de Meta desconectada')
      checkConnections()
      // Si no quedan cuentas, marcar como desconectado
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-fade-in">
      ...
      <ConnectionCard
        ...
        onDisconnect={() => metaAccounts.length > 0 && disconnectMeta(metaAccounts[0].id)}
            ...
            {metaAccounts.map(account => (
              <div key={account.id} className="flex items-center justify-between p-3 bg-dark-800/50 rounded-lg">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => disconnectMeta(account.id)}
        ...
        {!metaConnected && (
          <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-xl">
            <p className="text-sm text-blue-300">
              <strong>Nota:</strong> Se abrirá una ventana de Facebook para autorizar el acceso a tus cuentas publicitarias.
            </p>
          </div>
        )}
      </ConnectionCard>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full pl-4 pr-12 py-3 bg-dark-800 border border-dark-700 rounded-xl text-white placeholder-dark-400 focus:outline-none focus:border-lucid-500 focus:ring-2 focus:ring-lucid-500/20 transition-all duration-200 font-mono text-sm"
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <select
                  ...
                  className="w-full pl-4 pr-4 py-3 bg-dark-800 border border-dark-700 rounded-xl text-white focus:outline-none focus:border-lucid-500 focus:ring-2 focus:ring-lucid-500/20 transition-all duration-200"
                >
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full pl-4 pr-4 py-3 bg-dark-800 border border-dark-700 rounded-xl text-white placeholder-dark-400 focus:outline-none focus:border-lucid-500 focus:ring-2 focus:ring-lucid-500/20 transition-all duration-200"
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full pl-4 pr-12 py-3 bg-dark-800 border border-dark-700 rounded-xl text-white placeholder-dark-400 focus:outline-none focus:border-lucid-500 focus:ring-2 focus:ring-lucid-500/20 transition-all duration-200"
                    placeholder="••••••••"
                  />
                  <button
                    type="button"
