                ...
                <div className="glass rounded-2xl p-6">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      ...
                      <p className="text-xs text-dark-400">Solo entregas completadas y devoluciones</p>
                    </div>
                    <div className="flex items-center gap-4">
                      <div className="flex items-center gap-2">
                  ...
                  {/* Resumen Dropshipping - 5 columnas */}
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-4">
                    <div className="bg-emerald-500/10 rounded-xl p-3 text-center">
                      ...
                      <p className="text-[10px] text-emerald-400/60">{walletHistory.dropshipping.count_ganancias || 0} entregas</p>
                    </div>
                    <div className="bg-orange-500/10 rounded-xl p-3 text-center">
                      <p className="text-xs text-orange-400 mb-1">Devoluciones</p>
                      <p className="text-lg font-bold text-orange-400">{formatCurrency(walletHistory.dropshipping.total_devoluciones)}</p>
                      <p className="text-[10px] text-orange-400/60">{walletHistory.dropshipping.count_devoluciones || 0} devs</p>
                    </div>
                    <div className="bg-blue-500/10 rounded-xl p-3 text-center">
                      <p className="text-xs text-blue-400 mb-1">Prom. Entrega</p>
                      <p className="text-lg font-bold text-blue-400">{formatCurrency(walletHistory.dropshipping.promedio_ganancia || 0)}</p>
                    </div>
                    <div className="bg-purple-500/10 rounded-xl p-3 text-center">
                      <p className="text-xs text-purple-400 mb-1">Prom. Devolución</p>
                      <p className="text-lg font-bold text-purple-400">{formatCurrency(walletHistory.dropshipping.promedio_devolucion || 0)}</p>
                    </div>
                ...
                <div className="glass rounded-2xl p-6 border border-amber-500/20">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-dark-400">Comparación entre órdenes y pagos/cobros en wallet</p>
                    </div>
                  </div>
                  ...
                  {(() => {
                    ...
                    const avgGanancia = walletHistory.dropshipping?.promedio_ganancia || 0
                    const avgDevolucion = walletHistory.dropshipping?.promedio_devolucion || 0
                    
                    const entregasPendientesPago = ordersDelivered - walletGanancias
                    const devolucionesPendientesCobro = ordersReturned - walletDevoluciones
              ...
              {walletHistory.dropshipping && data?.summary && (
                <div className="glass rounded-2xl p-6 border border-lucid-500/20">
                  ...
                  {(() => {
                    ...
                    const avgGain = walletHistory.dropshipping.promedio_ganancia || 0
                    const avgReturn = walletHistory.dropshipping.promedio_devolucion || 0
                    const projectedGains = pendingToDeliver * avgGain
                    const projectedReturns = pendingToReturn * avgReturn
                    const adSpend = data.ads
                      ?.filter(ad => selectedProjectionCampaigns.includes(ad.campaign_name))
                      .reduce((sum, ad) => sum + (ad.spend || 0), 0) || 0
                    const currentUtility = walletHistory.dropshipping.utilidad_neta || 0
