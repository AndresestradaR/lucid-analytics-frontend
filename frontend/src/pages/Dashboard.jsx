...
function getCPAStatus(cpa) {
  if (!cpa || cpa <= 0) return { color: 'text-dark-400', bg: '', message: '', emoji: '' }
...
export default function Dashboard() {
  ...
  // Filtrar anuncios por campañas seleccionadas
  const getFilteredAds = () => {
    if (!data?.ads) return []
    if (selectedCampaigns.length === 0) return data.ads
    return data.ads.filter(ad => selectedCampaigns.includes(ad.campaign_name))
  }
        ...
        <>
          ...
          <div className="glass rounded-2xl p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              {selectedCampaigns.length > 0 && selectedCampaigns.length < availableCampaigns.length && (
                <span className="text-sm text-dark-400">
                  Mostrando {filteredAds.length} de {data.ads.length} anuncios
                </span>
              )}
            </div>
