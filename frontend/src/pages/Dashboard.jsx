          ...
          {ads.map((ad, index) => {
            ...
            return (
              <tr key={ad.ad_id} className="border-b border-dark-800 hover:bg-dark-800/50 transition-colors">
                ...
                <td className="text-center py-4 px-4">{ad.cpa > 0 && <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-bold ${cpaStatus.bg} ${cpaStatus.color}`}>{cpaStatus.emoji} {cpaStatus.message}</span>}</td>
...
export default function Dashboard() {
  ...
  useEffect(() => { if (selectedAccount) { loadDashboard(); loadDropiData(); loadWalletHistory() } }, [selectedAccount, dateRange])
  useEffect(() => { if (dropiData?.orders) { setProjectionPending(dropiData.orders.en_ruta || 0); setProjectionDeliveryRate(dropiData.orders.effective_delivery_rate || 70) } }, [dropiData])
  useEffect(() => {
    if (data?.ads) {
      const campaigns = [...new Set(data.ads.map(ad => ad.campaign_name).filter(Boolean))]
      setAvailableCampaigns(campaigns)
      if (selectedCampaigns.length === 0 && campaigns.length > 0) setSelectedCampaigns(campaigns)
      if (selectedStatuses.length === 0) setSelectedStatuses(CPA_STATUSES.map(s => s.id))
    }
  }, [data])
  ...
  const loadAccounts = async () => {
    try {
      ...
      const normalizedAccounts = rawAccounts.map(acc => ({ account_id: acc.account_id || acc.id, account_name: acc.account_name || acc.name, connected_at: acc.connected_at }))
      const filteredAccounts = normalizedAccounts.filter(acc => !acc.account_name?.includes('(Read-Only)'))
      setAccounts(filteredAccounts)
      if (filteredAccounts.length > 0) setSelectedAccount(filteredAccounts[0].account_id)
    } catch (err) { console.error('Error loading accounts:', err) }
  }
  const loadDashboard = async () => {
    setLoading(true); setError(null)
    try { const response = await api.get(`/analytics/dashboard?account_id=${selectedAccount}&start_date=${dateRange.start}&end_date=${dateRange.end}`); setData(response.data) }
  ...
  const loadWalletHistory = async () => {
    ...
    catch (err) { console.log('Wallet history error:', err); setWalletHistory(null) }
  }
  ...
  const getFilteredAds = () => {
    ...
    if (selectedCampaigns.length > 0) filtered = filtered.filter(ad => selectedCampaigns.includes(ad.campaign_name))
    if (selectedStatuses.length > 0 && selectedStatuses.length < CPA_STATUSES.length) filtered = filtered.filter(ad => selectedStatuses.includes(getAdStatusId(ad)))
    return filtered
  }
  ...
  const prepareDropiPieData = () => {
    ...
    return [{ name: 'Entregados', value: delivered || 0, color: '#22c55e' }, { name: 'Devueltos', value: returned || 0, color: '#ef4444' }, { name: 'En Ruta', value: en_ruta || 0, color: '#f59e0b' }, { name: 'Cancelados', value: cancelled || 0, color: '#6b7280' }, { name: 'Pend. Confirm.', value: pending_confirmation || 0, color: '#8b5cf6' }].filter(d => d.value > 0)
  }
  ...
  if (!selectedAccount && !loading) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center">
        <div className="p-4 rounded-2xl bg-amber-500/10 mb-4"><AlertCircle className="w-12 h-12 text-amber-400" /></div>
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-display font-bold text-white mb-1">Dashboard</h1>
          <p className="text-dark-400 text-sm">Métricas de tus campañas en tiempo real</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          {accounts.length > 0 && (
            <select value={selectedAccount} onChange={(e) => { setSelectedAccount(e.target.value); setSelectedCampaigns([]); setSelectedStatuses([]) }} className="px-3 py-2 bg-dark-800 border border-dark-700 rounded-xl text-white text-sm focus:outline-none focus:border-lucid-500 max-w-[180px]">
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-3">
              <div className="p-2 rounded-xl bg-blue-500/10"><Activity className="w-5 h-5 text-blue-400" /></div>
              <div><h3 className="font-display font-semibold text-white">Métricas Meta + LucidBot</h3><p className="text-xs text-dark-400">Facebook & Instagram Ads con ventas de WhatsApp</p></div>
            </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="glass rounded-2xl p-4"><p className="text-dark-400 text-xs mb-1">CPA Promedio</p><p className="text-lg font-display font-bold text-white">{formatCurrency(filteredSummary.average_cpa || 0)}</p></div>
            <div className="glass rounded-2xl p-4"><p className="text-dark-400 text-xs mb-1">ROAS Promedio</p><p className="text-lg font-display font-bold text-lucid-400">{(filteredSummary.average_roas || 0).toFixed(2)}x</p></div>
            <div className="glass rounded-2xl p-4"><p className="text-dark-400 text-xs mb-1">Profit</p><p className={`text-lg font-display font-bold ${filteredSummary.profit >= 0 ? 'text-lucid-400' : 'text-red-400'}`}>{formatCurrency(filteredSummary.profit || 0)}</p></div>
          </div>

          {/* Charts */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <div className="glass rounded-2xl p-5">
              <h3 className="font-display font-semibold text-white mb-5">Gasto vs Revenue por Anuncio</h3>
          ...
          <div className="glass rounded-2xl p-5">
            <div className="flex items-center justify-between mb-5">
              ...
              <span className="text-xs text-dark-400">{filteredAds.length} de {data.ads?.length || 0} anuncios</span>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4 pt-4">
                <div className="flex items-center gap-3">
                  ...
                  <div><h3 className="font-display font-semibold text-white">Métricas Dropi</h3><p className="text-xs text-dark-400">Pedidos y fulfillment</p></div>
                </div>
          ...
          {walletHistory?.dropshipping && data?.summary && (
            <div className="glass rounded-2xl p-5 border border-lucid-500/20">
              ...
              {(() => {
                ...
                const adSpend = data.ads?.filter(ad => selectedProjectionCampaigns.includes(ad.campaign_name)).reduce((sum, ad) => sum + (ad.spend || 0), 0) || 0
