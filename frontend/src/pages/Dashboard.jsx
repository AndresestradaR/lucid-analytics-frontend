        ...
        <span>
          {selectedCount === 0 
            ...
            : selectedCount === totalCount 
              ? 'Todas las campañas' 
              : `${selectedCount} campaña${selectedCount > 1 ? 's' : ''}`
          }
        </span>
        ...
        <div className="absolute z-50 mt-2 w-80 bg-dark-800 border border-dark-700 rounded-xl shadow-xl overflow-hidden">
          ...
          <div className="flex items-center justify-between p-3 border-b border-dark-700">
            ...
            <div className="flex gap-2">
              <button 
                onClick={selectAll}
                className="text-xs text-lucid-400 hover:text-lucid-300"
              >
                Todas
              </button>
...
function StatusFilter({ selectedStatuses, onChange, adCounts }) {
  ...
  const selectAll = () => {
    onChange(CPA_STATUSES.map(s => s.id))
  }
...
// Función para obtener el color y mensaje del semáforo según CPA
...
// Función para obtener el ID de estado de un anuncio
            ...
            return (
              <tr key={ad.ad_id} className="border-b border-dark-800 hover:bg-dark-800/50 transition-colors">
                ...
                <td className="text-right py-4 px-4 text-white text-sm">
                  {ad.daily_budget ? (
                    <span className="text-purple-400">${formatNumber(ad.daily_budget)}/día</span>
                  ) : ad.lifetime_budget ? (
                    <span className="text-purple-400">${formatNumber(ad.lifetime_budget)} total</span>
                  ) : '-'}
                </td>
                ...
                <td className="text-right py-4 px-4 text-white">
                  {ad.cpm ? formatCurrency(ad.cpm) : '-'}
                </td>
                <td className="text-right py-4 px-4 text-white">
                  {ad.messaging_conversations || '-'}
                </td>
                <td className="text-right py-4 px-4 text-white">
                  {ad.cost_per_messaging > 0 ? formatCurrency(ad.cost_per_messaging) : '-'}
                </td>
                ...
                <td className="text-right py-4 px-4">
                  <span className={`font-bold ${cpaStatus.color}`}>
                    {ad.cpa > 0 ? formatCurrency(ad.cpa) : '-'}
                  </span>
                </td>
                <td className="text-center py-4 px-4">
                  {ad.cpa > 0 && (
                    <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-bold ${cpaStatus.bg} ${cpaStatus.color}`}>
                      {cpaStatus.emoji} {cpaStatus.message}
                    </span>
                  )}
                </td>
                <td className="text-right py-4 px-4">
...
export default function Dashboard() {
  ...
  // Actualizar campañas disponibles cuando cambian los datos
  useEffect(() => {
    if (data?.ads) {
      const campaigns = [...new Set(data.ads.map(ad => ad.campaign_name).filter(Boolean))]
      setAvailableCampaigns(campaigns)
      // Por defecto, seleccionar todas las campañas
      if (selectedCampaigns.length === 0 && campaigns.length > 0) {
        setSelectedCampaigns(campaigns)
      }
      // Por defecto, seleccionar todos los estados
  ...
  const loadAccounts = async () => {
    try {
      ...
      // FILTRAR cuentas que contienen "(Read-Only)"
      const filteredAccounts = (response.data.accounts || []).filter(
        acc => !acc.account_name.includes('(Read-Only)')
      )
  ...
  // Contar anuncios por estado (para mostrar en el filtro)
  ...
  // Filtrar anuncios por campañas Y estados seleccionados
          ...
          {accounts.length > 0 && (
            <select
              ...
              onChange={(e) => {
                ...
                setSelectedCampaigns([]) // Reset campaign filter on account change
                setSelectedStatuses([]) // Reset status filter on account change
              }}
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard
              title="Gasto Total"
              value={filteredSummary.total_spend}
              icon={DollarSign}
              prefix="$"
            />
            <StatCard
              title="Revenue"
              value={filteredSummary.total_revenue}
              icon={TrendingUp}
              prefix="$"
              change={filteredSummary.total_revenue > filteredSummary.total_spend ? 'up' : 'down'}
              trend={filteredSummary.total_revenue > filteredSummary.total_spend ? 'up' : 'down'}
            />
            <StatCard
              title="Leads"
              value={filteredSummary.total_leads}
              icon={Users}
            />
            <StatCard
              title="Ventas"
              value={filteredSummary.total_sales}
              icon={ShoppingCart}
            />
          </div>
          ...
          <div className="glass rounded-2xl p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <span className="text-sm text-dark-400">
                Mostrando {filteredAds.length} de {data.ads?.length || 0} anuncios
              </span>
            </div>
