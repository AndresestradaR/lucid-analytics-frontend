        ...
        <>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-display font-semibold text-white">Métricas Meta + LucidBot</h3>
                <p className="text-xs text-dark-400">Facebook & Instagram Ads con ventas de WhatsApp</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="glass rounded-2xl p-6">
              ...
              <p className={`text-2xl font-display font-bold ${filteredSummary.profit >= 0 ? 'text-lucid-400' : 'text-red-400'}`}>
                {formatCurrency(filteredSummary.profit || 0)}
              </p>
            </div>
          </div>
          ...
          {dropiData?.connected && (
            <div className="space-y-4">
              ...
              <div className="glass rounded-2xl p-6">
                <h3 className="font-display font-semibold text-white mb-6">Estado de Pedidos</h3>
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-display font-semibold text-white">Flujo de Caja Real</h3>
                    <p className="text-xs text-dark-400">Cuándo entró/salió el dinero de tu wallet (puede incluir pagos de pedidos anteriores)</p>
                  </div>
                </div>
              ...
              {/* Gráfica de Ganancias Dropshipping vs Devoluciones */}
              {dropiData?.orders && (
                <div className="glass rounded-2xl p-6">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <h3 className="font-display font-semibold text-white">Rentabilidad por Pedido</h3>
                      <p className="text-xs text-dark-400">Basado en fecha de creación del pedido (sin LAG)</p>
                    </div>
                    <div className="flex items-center gap-4">
                      <div className="flex items-center gap-2">
                  ...
                  {/* Resumen con CRUCE DE DATOS */}
                  {dropiData?.reconciliation ? (
                    <>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
                        <div className="bg-emerald-500/10 rounded-xl p-3 text-center border border-emerald-500/20">
                          <p className="text-xs text-emerald-400 mb-1">✅ Ganancias Cobradas</p>
                          <p className="text-lg font-bold text-emerald-400">{formatCurrency(dropiData.reconciliation.entregas_cobradas_monto)}</p>
                          <p className="text-[10px] text-emerald-400/60">{dropiData.reconciliation.entregas_cobradas} entregas pagadas</p>
                        </div>
                        <div className="bg-emerald-500/5 rounded-xl p-3 text-center border border-emerald-500/10 border-dashed">
                          <p className="text-xs text-emerald-300/70 mb-1">⏳ Ganancias Pendientes</p>
                          <p className="text-lg font-bold text-emerald-300/70">{formatCurrency(dropiData.reconciliation.entregas_pendientes_monto)}</p>
                          <p className="text-[10px] text-emerald-300/50">{dropiData.reconciliation.entregas_pendientes} por cobrar</p>
                        </div>
                        <div className="bg-orange-500/10 rounded-xl p-3 text-center border border-orange-500/20">
                          <p className="text-xs text-orange-400 mb-1">✅ Devs Cobradas</p>
                          <p className="text-lg font-bold text-orange-400">{formatCurrency(dropiData.reconciliation.devoluciones_cobradas_monto)}</p>
                          <p className="text-[10px] text-orange-400/60">{dropiData.reconciliation.devoluciones_cobradas} descontadas</p>
                        </div>
                        <div className="bg-orange-500/5 rounded-xl p-3 text-center border border-orange-500/10 border-dashed">
                          <p className="text-xs text-orange-300/70 mb-1">⏳ Devs Pendientes</p>
                          <p className="text-lg font-bold text-orange-300/70">{formatCurrency(dropiData.reconciliation.devoluciones_pendientes_monto)}</p>
                          <p className="text-[10px] text-orange-300/50">{dropiData.reconciliation.devoluciones_pendientes} por descontar</p>
                        </div>
                      </div>
                      
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
                        <div className="bg-green-500/10 rounded-xl p-3 text-center">
                          <p className="text-xs text-green-400 mb-1">Total Ganancias</p>
                          <p className="text-lg font-bold text-green-400">{formatCurrency(dropiData.reconciliation.total_ganancias)}</p>
                        </div>
                        <div className="bg-red-500/10 rounded-xl p-3 text-center">
                          <p className="text-xs text-red-400 mb-1">Total Devoluciones</p>
                          <p className="text-lg font-bold text-red-400">{formatCurrency(dropiData.reconciliation.total_devoluciones)}</p>
                        </div>
                        <div className={`${dropiData.reconciliation.utilidad_cobrada >= 0 ? 'bg-cyan-500/10 border-cyan-500/20' : 'bg-red-500/10 border-red-500/20'} rounded-xl p-3 text-center border`}>
                          <p className={`text-xs ${dropiData.reconciliation.utilidad_cobrada >= 0 ? 'text-cyan-400' : 'text-red-400'} mb-1`}>💰 Utilidad Cobrada</p>
                          <p className={`text-lg font-bold ${dropiData.reconciliation.utilidad_cobrada >= 0 ? 'text-cyan-400' : 'text-red-400'}`}>{formatCurrency(dropiData.reconciliation.utilidad_cobrada)}</p>
                        </div>
                        <div className={`${dropiData.reconciliation.pendiente_neto >= 0 ? 'bg-blue-500/10 border-blue-500/20' : 'bg-amber-500/10 border-amber-500/20'} rounded-xl p-3 text-center border border-dashed`}>
                          <p className={`text-xs ${dropiData.reconciliation.pendiente_neto >= 0 ? 'text-blue-400' : 'text-amber-400'} mb-1`}>
                            {dropiData.reconciliation.pendiente_neto >= 0 ? '📈 Por Recibir' : '📉 Por Descontar'}
                          </p>
                          <p className={`text-lg font-bold ${dropiData.reconciliation.pendiente_neto >= 0 ? 'text-blue-400' : 'text-amber-400'}`}>{formatCurrency(Math.abs(dropiData.reconciliation.pendiente_neto))}</p>
                        </div>
                      </div>
                      
                      <div className={`mb-4 p-4 rounded-xl ${dropiData.reconciliation.utilidad_neta >= 0 ? 'bg-gradient-to-r from-lucid-500/20 to-cyan-500/20 border border-lucid-500/30' : 'bg-gradient-to-r from-red-500/20 to-orange-500/20 border border-red-500/30'}`}>
                        <div className="flex items-center justify-between">
                          <div>
                            <p className={`text-sm ${dropiData.reconciliation.utilidad_neta >= 0 ? 'text-lucid-400' : 'text-red-400'}`}>
                              {dropiData.reconciliation.utilidad_neta >= 0 ? '🎯 UTILIDAD NETA DEL PERÍODO' : '⚠️ PÉRDIDA DEL PERÍODO'}
                            </p>
                            <p className="text-[10px] text-dark-400">Cobrada + Pendiente = Total</p>
                          </div>
                          <p className={`text-2xl font-bold ${dropiData.reconciliation.utilidad_neta >= 0 ? 'text-lucid-400' : 'text-red-400'}`}>
                            {formatCurrency(dropiData.reconciliation.utilidad_neta)}
                          </p>
                        </div>
                      </div>
                      ...
                      {dropiData.reconciliation.en_ruta > 0 && (
                        <div className="mb-4 p-3 rounded-xl bg-yellow-500/10 border border-yellow-500/20">
                          <p className="text-sm text-yellow-400">
                            🚚 <strong>{dropiData.reconciliation.en_ruta}</strong> pedidos en ruta por valor potencial de <strong>{formatCurrency(dropiData.reconciliation.en_ruta_monto)}</strong>
                          </p>
                        </div>
                      )}
                    </>
                    ...
                    <div className="p-4 rounded-xl bg-dark-700/50 text-center">
                      <p className="text-dark-400">Cargando datos de conciliación...</p>
                    </div>
                  )}
                  ...
                  {dropiData?.daily_reconciled && dropiData.daily_reconciled.length > 0 && (
                    <ResponsiveContainer width="100%" height={280}>
                      <AreaChart data={dropiData.daily_reconciled}>
                        <defs>
                          <linearGradient id="colorGananciasCobradas" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="5%" stopColor="#10b981" stopOpacity={0.6}/>
                            <stop offset="95%" stopColor="#10b981" stopOpacity={0}/>
                          </linearGradient>
                          <linearGradient id="colorGananciasPendientes" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="5%" stopColor="#22d3ee" stopOpacity={0.4}/>
                            <stop offset="95%" stopColor="#22d3ee" stopOpacity={0}/>
                          </linearGradient>
                          <linearGradient id="colorDevolucionesCobradas" x1="0" y1="0" x2="0" y2="1">
                        ...
                        <Tooltip 
                          ...
                          formatter={(value, name) => {
                            const labels = {
                              'ganancias_cobradas': '✅ Ganancias Cobradas',
                              'ganancias_pendientes': '⏳ Ganancias Pendientes',
                              'devoluciones_cobradas': '↩️ Devoluciones Cobradas'
                            }
                        ...
                        <Area 
                          type="monotone" 
                          dataKey="ganancias_cobradas" 
                          stroke="#10b981" 
                          strokeWidth={2}
                          fillOpacity={1} 
                          fill="url(#colorGananciasCobradas)" 
                          stackId="ganancias"
                          dot={{ r: 2, fill: '#10b981' }}
                        />
                        <Area 
                          type="monotone" 
                          dataKey="ganancias_pendientes" 
                          stroke="#22d3ee" 
                          strokeWidth={2}
                          strokeDasharray="5 5"
                          fillOpacity={1} 
                          fill="url(#colorGananciasPendientes)" 
                          stackId="ganancias"
                          dot={{ r: 2, fill: '#22d3ee' }}
                        />
                        <Area 
                          type="monotone" 
                          dataKey="devoluciones_cobradas" 
                          stroke="#f97316" 
                          strokeWidth={2}
                          fillOpacity={1} 
                          fill="url(#colorDevolucionesCobradas)" 
                          dot={{ r: 2, fill: '#f97316' }}
                        />
                      </AreaChart>
                    </ResponsiveContainer>
